...
namespace unrealenginedecryption
{
	...
	public:
		...
		{
			std::string result;
			uintptr_t test_platform = read<uintptr_t>( PlayerState + 0x3f0);
			wchar_t platform[ 64 ];
			//read_physical( ( PVOID )test_platform, platform, sizeof( platform ) );
		...
		__forceinline std::string username( uintptr_t playerstate )
		{
			...
			uintptr_t pNameStructure = read < uintptr_t >( playerstate + 0xA98); //next offset 0xAF8
			if ( is_valid( pNameStructure ) ) {
				...
				if ( is_valid( pNameEncryptedBuffer ) ) {
					//read_physical( (PVOID)pNameEncryptedBuffer, pNameBuffer, pNameLength );
					...
					if ( is_valid( ( uint64_t )pNameBuffer ) )
					{
		...
		__forceinline std::string weapon( uint64_t CurrentWeapon ) {
			...
			uint64_t ItemName = read<uint64_t>( weapondata + 0x38 );
			if ( !ItemName ) return "";

			uint64_t FData = read<uint64_t>( ItemName + 0x28 );
			int FLength = read<int>( ItemName + 0x40 );

			if ( FLength > 0 && FLength < 50 ) {

				wchar_t* WeaponBuffer = new wchar_t[ FLength ];
				//read_physical( (PVOID)FData, ( PVOID )WeaponBuffer, FLength * sizeof( wchar_t ) );
