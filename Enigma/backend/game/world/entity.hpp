...
namespace loop
{
	...
	public:
		...
		__forceinline auto cache( ) -> void
		{
			...
			for ( int i = 0; i < 25; i++ )
			{
				if ( read<__int32>( device.image_base + ( i * 0x1000 ) + 0x250 ) == 0x260E020B )
				{
					va_text = device.image_base + ( ( i + 1 ) * 0x1000 );
				}
			}*/

			cached->gworld = read<uintptr_t>( va_text + offsets->gworld );
			...
			cached->velocity = read<Vector3>( cached->rootcomp + offsets->velocity );

			cached->holdingweapon = read<uintptr_t>( cached->acknowledgedpawn + offsets->currentweapon );
			...
			for ( int i = 0; i < cached->entitycount; ++i )
			{
				...
				auto player = read<uintptr_t>( cached->entityarray + ( i * sizeof( uintptr_t ) ) );

				auto current_actor = read<uintptr_t>( player + offsets->pawnpriv );


				auto rootcomp = read<uintptr_t>( current_actor + offsets->rootcomp );
				auto location = read <Vector3>( rootcomp + offsets->relativelocation );

				if ( ( !uengine->screencheck( uengine->w2s( location ) ) ) && ( !read<uintptr_t>( current_actor + offsets->vehicle ) ) ) continue;
				...
				FortniteActor.TeamIndex = read<int>( player + offsets->teamid );

				FortniteActor.CurrentWeapon = read<uintptr_t>( player + offsets->currentweapon );
				...
				FortniteActor.SkeletalMesh = read<uintptr_t>( current_actor + offsets->mesh );

				FortniteActor.PlayerState = read<uintptr_t>( current_actor + offsets->playerstate );
		...
		__forceinline auto loop( ) -> void
		{
			...
			for ( Actors Actor : ActorList )
			{
				...
				//std::cout << CornerHeight << std::endl;
				...
				/*if ()
				{
					uintptr_t AActorRootComponent = read<uintptr_t>(Actor.Actor + offsets->rootcomp);
