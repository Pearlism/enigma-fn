...
namespace a
{
	...
	public:
		...
		__forceinline auto aimbot_function(uintptr_t entity, uintptr_t gworld, uintptr_t playercontroller, uintptr_t pawnprivate, Vector3 velocity, Vector3 loc, std::string weapon) -> void
		{
			...
			auto test = read<uintptr_t>(entity + offsets->rootcomp);
			auto Velocity = read<Vector3>(test + offsets->velocity);


			uintptr_t currentactormesh = read<uintptr_t>(entity + offsets->mesh);
			...
			if (bone.x != 0 || bone.y != 0)
			{
				...
				if ((aimbot->triggerbot) && (weapon.find(_("Shotgun")) != std::string::npos) && (GetAsyncKeyState(aimbot->triggerkey) < 0)) {
					...
					if ((read<uintptr_t>(playercontroller + offsets->targetedpawn))) //TargetedFortPawn
					{
						if (has_clicked) {
				...
				if (aimbot->playerfreeze)
				{
					...
					{
						if (write<float>(pawnprivate + 0x68, 0.0f))
						{
							//	std::cout << "black\n";
						}//CustomTimeDilation;
					}
					...
					{
						write<float>(pawnprivate + 0x68, 1.0f); //CustomTimeDilation;
					}
				}
			}
		}
