...
struct o
{
	...
	uintptr_t mesh = 0x310; //struct USkeletalMeshComponent* Mesh;

	uintptr_t vehicle = 0x2960; //struct AActor* CurrentVehicle;

	uintptr_t teamid = 0x1201; //char TeamIndex; (AFortPlayerStateAthena : AFortPlayerStateZone)

	uintptr_t weapondata = 0x510; //struct UFortWeaponItemDefinition* WeaponData;

	uintptr_t displayname = 0x98; //??

	uintptr_t length = 0x40; //??

	uintptr_t currentweapon = 0x9d8; //struct AFortWeapon* CurrentWeapon;

	uintptr_t comptowrld = 0x1c0; // NOT IN SDK

	uintptr_t bonearray = 0x570;// NOT IN SDK

	uintptr_t bonecache = 0x5B8; // NOT IN SDK

	uintptr_t bonec = 0x60;// NOT IN SDK

	uintptr_t targetedpawn = 0x1898;

	uintptr_t namepoolchunk = 0x10EFDA00; // NOT IN SDK

	uintptr_t lastsubmit = 0x2E0; // NOT IN SDK
	uintptr_t lastrender = 0x2E8; // NOT IN SDK
